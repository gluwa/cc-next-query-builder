...
export function safeGetFieldsForType1(tx: TransactionResponse): EncodedFields {
  const out = {
    ...
    values: [
      ...
      tx.value,
      tx.data,
      safeEncodeAccessList(tx.accessList),
...
export function safeGetFieldsForType(tx: TransactionResponse): EncodedFields {
  switch (tx.type) {
    case 0:
      return safeGetFieldsForType0(tx);
    case 1:
      return safeGetFieldsForType1(tx);
    case 2:
      return safeGetFieldsForType2(tx);
    case 3:
      return safeGetFieldsForType3(tx);
    default:
...
function safeGetReceiptFields(rx: TransactionReceipt): EncodedFields {
  const fields = {
    types: ['uint8', 'uint64', 'bytes[]', 'bytes'],
    values: [rx.status, rx.gasUsed, rx.logs.map(safeEncodeLog), rx.logsBloom],
  };
